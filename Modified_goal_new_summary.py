...
def build_macro_context(macro, news):
    ...
    weekly_features = macro.get("weekly_fx_features", {})
    ...
    if weekly_features and not weekly_features.get("note"):
        weekly_lines = []
        for key, info in weekly_features.items():
            if key in {"latest_week", "row_count", "note"}:
                continue
            if isinstance(info, dict):
                ...
                if latest is not None:
                    weekly_lines.append(f"{key} : {latest} ({trend}, change {change})")
